## 02b_simulate_and_recover_fast.R
## Alternative reduced-scale estimation: 60 subjects, correlated omega,
## 100 burn-in and 100 estimation iterations. Not the source of the
## reported recovery table; see R/README.md for the diagonal-omega run.
## Synthetic estimation demonstration; patient-level trial data are not public.
## RUN FROM THE REPOSITORY ROOT: source("R/02b_simulate_and_recover_fast.R")
...
## DIAGONAL-ONLY simplification: the covariance terms between random effects
## are dropped. With only 60 subjects and Day 1/8-only sampling per subject,
## the eta.cl-eta.v2 covariance is weakly identified and destabilizes the
## SAEM covariance step (observed: "system is singular" warnings, the
## covariance term pinned at 0 then jumping unstably). Diagonal IIV is a
## legitimate simplification for THIS demonstration, whose purpose is to
## show fixed effects and marginal IIV recover correctly -- it does not
## claim to reproduce Amgen's full published covariance structure.
omega_true <- lotri({
...
RICH <- FALSE
n_subj <- 60   # reduced-scale demonstration
...
fit <- nlmixr2(est, dat, est = "saem",
               control = saemControl(nBurn = 100, nEm = 100, print = 20, seed = 1))
...
rec_omega <- data.frame(
  element = c("omega V2", "omega CL", "omega Ka"),
  truth = c(P[["OMEGA_1_1"]], P[["OMEGA_2_2"]], P[["OMEGA_3_3"]]),
  estimate = c(om["eta.v2","eta.v2"], om["eta.cl","eta.cl"], om["eta.ka","eta.ka"])
)
cat("\nNOTE: this run uses a DIAGONAL omega (covariance terms fixed to 0),\n")
cat("a simplification made for numerical stability at n=60 with sparse\n")
cat("per-subject sampling. The published model's covariance terms\n")
cat("(omega V2-CL correlation ~67%) are NOT estimated here. State this\n")
cat("plainly if this run is the one used in the write-up.\n")
